		...
		*/    
	    ...
	    try{	
	     PlanRepeat pr = PlanRepeat.setUp("sendCommandStart",-1);
	    	...
	    	temporaryStr = "\"Command sended\"";
	    	println( temporaryStr );  
	    	temporaryStr = QActorUtils.unifyMsgContent(pengine,"startRetrievingCmd","startRetrievingCmd", guardVars ).toString();
	    	sendMsg("startRetrievingCmd","robot_retriever", QActorContext.dispatch, temporaryStr ); 
	    	//switchTo handleMsg
	        switchToPlanAsNextState(pr, myselfName, "map_keeper_"+myselfName, 
	              "handleMsg",false, false, null); 
	    ...
	    try{	
	     PlanRepeat pr = PlanRepeat.setUp("handleBombPosition",-1);
	    	...
	    	if( (guardVars = QActorUtils.evalTheGuard(this, " ??bombPosition(X,Y)" )) != null ){
	    	temporaryStr = QActorUtils.unifyMsgContent(pengine,"bombPositionMsg(X,Y)","bombPositionMsg(X,Y)", guardVars ).toString();
	    	sendMsg("bombPositionMsg","robot_retriever", QActorContext.dispatch, temporaryStr ); 
	    	}
	    	//bbb
	     msgTransition( pr,myselfName,"map_keeper_"+myselfName,false,
	          new StateFun[]{}, 
