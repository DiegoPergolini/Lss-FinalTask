...
changeEnvCond( T):-replaceRule( envCond( _),envCond( T)),output( envCond( T)).
obstacleCell( X,Y,downDir,X,Y1):-eval( plus,Y,1,Y1).
obstacleCell( X,Y,upDir,X,Y1):-eval( minus,Y,1,Y1).
obstacleCell( X,Y,leftDir,X1,Y):-eval( minus,X,1,X1).
obstacleCell( X,Y,rightDir,X1,Y):-eval( plus,X,1,X1).
